...
- El ETL escribe Bronze/Silver en processzone y carga Gold a BigQuery usando el connector.
...
Región:
- Si tu dataset está creado en us-east1, déjalo así (no “US” multi-region).
- El staging bucket puede estar en us-east1 también (recomendado).

## 4) Cómo correr (end-to-end)
### 4.1 Subir reference (si no hay uno reciente)
Sube un solo archivo dentro del ingestion_date correspondiente:

gsutil cp gdelt_country_risk_YYYYMMDD.csv \
  gs://bsg-gcs-landingzone/new-risk-monitor/gdelt/reference/country_risk/ingestion_date=YYYY-MM-DD/gdelt_country_risk_YYYYMMDD.csv

### 4.2 Subir events (esto dispara el pipeline)
...
2) Filtra para procesar SOLO si el objeto está bajo:
   new-risk-monitor/gdelt/landing/events/ingestion_date=YYYY-MM-DD/<file>
3) Extrae ingestion_date del path
4) Busca el reference “más reciente” bajo:
   new-risk-monitor/gdelt/reference/country_risk/ingestion_date=YYYY-MM-DD/<file>
...
- Trigger se dispara “de más”:
  - Con provider directo de Storage no hay filtro por prefijo; por eso usamos Audit Logs + path pattern.
